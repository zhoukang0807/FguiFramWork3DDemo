...
func init() {
	...
	handler(&msg.StatusDTO{}, handleSendStatus)
	handler(&msg.StatusResult{}, handleGetStatus)
}
...
func handleHello(args []interface{}) {
	...
	// 输出收到的消息的内容
	log.Release("hello %v", m.GetUsername())
...
func handleSendStatus(args []interface{}) {
	// 收到的 Hello 消息
	m := args[0].(*msg.StatusDTO)
	// 消息的发送者
	a := args[1].(gate.Agent)

	log.Debug("id", m.GetId())
	move:=msg.SessionStatusServ.SetStatus(m)
	// 给发送者回应一个 Hello 消息
	a.WriteMsg(move)
}

func handleGetStatus(args []interface{}) {
	// 收到的 Hello 消息
	// m := args[0].(*msg.MoveResult)
	// 消息的发送者
	a := args[1].(gate.Agent)

	var states []*msg.StatusDTO
	states = msg.SessionStatusServ.GetStatus()
	// 给发送者回应一个 Hello 消息
	a.WriteMsg(&msg.StatusResult{
		Status : states,
	})
}